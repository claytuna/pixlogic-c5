...
namespace Application\Block\PixIntroSection;
...
class Controller extends BlockController
{
    ...
    protected $btTable = 'btPixIntroSection';
    ...
    {
        return t("Adds an introduction to the page with title, description, and image");
    }

    public function getBlockTypeName()
    {
        return t("Intro Section - piXlogic");
    }
    ...
    public function getImageFeatureDetailFileObject()
    {
        ...
        $fID = $db->fetchColumn('select fID from btPixIntroSection where bID = ?', array($this->bID), 0);
        if ($fID) {
            $f = File::getByID($fID);
            if (is_object($f) && !$f->isError()) {
                return $f;
