...
namespace Application\Block\PixContact;
...
class Controller extends BlockController
{
    ...
    protected $btTable = 'btPixContact';
    ...
    {
        return t("Adds a contact info block with an image, phone, email, address");
    }

    public function getBlockTypeName()
    {
        return t("Contact Info - piXlogic");
    }
    ...
    public function getImageFeatureDetailFileObject()
    {
        ...
        $fID = $db->fetchColumn('select fID from btPixContact where bID = ?', array($this->bID), 0);
        if ($fID) {
            $f = File::getByID($fID);
            if (is_object($f) && !$f->isError()) {
                return $f;
